...
# Report Laboratories 9

Maciej Janicki 156073
Jakub Kubiak 156049

[Source code](https://github.com/majanicki/ec/tree/trunk/lab09)
...
In this laboratory the goal is to implement hybrid evolutionary algorithm and compare it with previously implemented methods of solving TSP: Multi start local search (MSLS), iterated local search (ILS) and large neighborhood search (LNS).

# Pseudocode

```
FUNCTION get_initial_population(size, dataset, dist)
    population := null
    FOR i IN 0..size:
        rand_solution := get_random_solution(dataset)
        ls_solution := get_local_search_steepest(rand_solution, 
                                                 dataset, 
                                                 dist, 
...
FUNCTION get_hybrid_evolution(dataset, dist, evo_operator)
    ...
    WHILE elapsed_time < 2.943:
        parent1 := RANDOM_ELEMENT(population)
        parent2 := RANDOM_ELEMENT(population)

        IF parent1 == parent2:
            parent2 = (parent2 + 1) % population.size();
        
        IF evo_operator == OPERATOR_1:
            offspring := evolution_operator1(parent1, parent2, dataset)
            offspring := get_local_search_steepest(offspring, 
                                                   dataset, 
                                                   dist, 
                                                   INTRA_ROUTE_EDGE_EXCHANGE)

        ELSE IF evo_operator == OPERATOR_2_LS:
            offspring := evolution_operator2(parent1, parent2, dataset, dist)
            offspring := get_local_search_steepest(offspring, 
                                                   dataset, 
                                                   dist, 
                                                   INTRA_ROUTE_EDGE_EXCHANGE)
        ...
        worst_cost = 0
        offspring_cost := compute_total_cost(offspring, dist)
        IF EXISTS sol IN population SUCH THAT compute_total_cost(sol) == offspring_cost:
            CONTINUE
        
        worst := SOLUTION_WITH_MAX_COST(population)
